...
abstract contract Streams {
    ...
    constructor(uint32 cycleSecs, bytes32 streamsStorageSlot) {
        ...
        _CYCLE_SECS = cycleSecs;
        _MIN_AMT_PER_SEC = (_AMT_PER_SEC_MULTIPLIER + cycleSecs - 1) / cycleSecs;
        _STREAMS_STORAGE_SLOT = streamsStorageSlot;
    }
        ...
        )
    {
        unchecked {
            ...
            for (uint32 cycle = fromCycle; cycle < toCycle; cycle++) {
                ...
                // amtPerCycle fits in int128, safe to cast
    ...
    function _squeezeStreams(
        ...
        unchecked {
            ...
            // amt * _AMT_PER_SEC_MULTIPLIER fits in int160
            // forge-lint: disable-next-line(unsafe-typecast)
            int160 delta = -int160(amt * _AMT_PER_SEC_MULTIPLIER);
            _addDeltaRange(state, cycleStart, cycleStart + 1, delta);
    ...
    function _squeezedAmt(
        ...
        unchecked {
            ...
            // amt fits in uint128 per protocol invariants
            // forge-lint: disable-next-line(unsafe-typecast)
            return uint128(amt);
        }
    }
    ...
    function _calcBalance(
        ...
        unchecked {
            ...
            for (uint256 i = 0; i < receivers.length; i++) {
                ...
                // _streamedAmt result fits in uint128 per protocol invariants
                // forge-lint: disable-next-line(unsafe-typecast)
    ...
    function _setStreams(
        ...
        unchecked {
            ...
            uint32 newMaxEnd;
            {
                ...
                // _calcBalance result is capped to state.balance which fits in int128
                // forge-lint: disable-next-line(unsafe-typecast)
                ...
                // currBalance + realBalanceDelta bounded by _MAX_STREAMS_BALANCE
                // forge-lint: disable-next-line(unsafe-typecast)
    ...
    function _calcMaxEnd(
        ...
        if (configsLen == 0 || balance == 0) {
            // enoughEnd is block.timestamp which fits in uint32
            // forge-lint: disable-next-line(unsafe-typecast)
        ...
        if (_isBalanceEnough(balance, configs, configsLen, notEnoughEnd)) {
            // notEnoughEnd is type(uint32).max which fits in uint32
            // forge-lint: disable-next-line(unsafe-typecast)
        ...
        while (true) {
            ...
            if (end == enoughEnd) {
                // end is bounded between enoughEnd and type(uint32).max
                // forge-lint: disable-next-line(unsafe-typecast)
                return uint32(end);
            }
    ...
    function _addConfig(
        ...
        require(amtPerSec >= _MIN_AMT_PER_SEC, "Stream receiver amtPerSec too low");
        (uint32 start, uint32 end) =
        ...
        returns (uint256 amtPerSec, uint256 start, uint256 end)
    {
        ...
        // By casting down we get value:
        // `start (32 bits)`
        // upper 32 bits shifted out, lower 32 bits fit in uint32
        // forge-lint: disable-next-line(unsafe-typecast)
        start = uint32(config >> 32);
        // By casting down we get value:
        // `end (32 bits)`
        // lower 32 bits fit in uint32
        // forge-lint: disable-next-line(unsafe-typecast)
        end = uint32(config);
    }
    ...
    function _streamRange(
        ...
        // end bounded by maxEnd which fits in uint32
        // forge-lint: disable-next-line(unsafe-typecast)
    ...
    function _addDelta(
        ...
        unchecked {
            ...
            // _AMT_PER_SEC_MULTIPLIER (1e9) fits in int160
            // forge-lint: disable-next-line(unsafe-typecast)
            int256 amtPerSecMultiplier = int160(_AMT_PER_SEC_MULTIPLIER);
            int256 fullCycle = (int256(uint256(_CYCLE_SECS)) * amtPerSec) / amtPerSecMultiplier;
            // slither-disable-next-line weak-prng
            // timestamp % _CYCLE_SECS fits in int256
            // forge-lint: disable-next-line(unsafe-typecast)
            int256 nextCycle = (int256(timestamp % _CYCLE_SECS) * amtPerSec) / amtPerSecMultiplier;
            // timestamp fits in uint32
            // forge-lint: disable-next-line(unsafe-typecast)
            ...
            // fullCycle - nextCycle bounded by total balance invariants
            // forge-lint: disable-next-line(unsafe-typecast)
            amtDelta.thisCycle += int128(fullCycle - nextCycle);
            // nextCycle bounded by total balance invariants
            // forge-lint: disable-next-line(unsafe-typecast)
    ...
    function _cycleOf(uint32 timestamp) private view returns (uint32 cycle) {
        unchecked {
            return timestamp / _CYCLE_SECS + 1;
        }
    }
    ...
    function _currCycleStart() private view returns (uint32 timestamp) {
        unchecked {
            ...
            // slither-disable-next-line weak-prng
            return currTimestamp - (currTimestamp % _CYCLE_SECS);
        }
    }
    ...
    function _streamsStorage() private view returns (StreamsStorage storage streamsStorage) {
        bytes32 slot = _STREAMS_STORAGE_SLOT;
        // slither-disable-next-line assembly
        assembly {

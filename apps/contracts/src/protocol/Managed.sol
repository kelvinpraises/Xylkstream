...
abstract contract Managed is UUPSUpgradeable {
    ...
    bytes32 private immutable _MANAGED_STORAGE_SLOT = _erc1967Slot("eip1967.managed.storage");
    ...
    function _onlyAdminOrConstructor() internal view {
        if (Address.isContract(address(this))) {
            require(admin() == msg.sender, "Caller not the admin");
        }
    }

    function _onlyAdminOrPauser() internal view {
    ...
    function _managedStorage() internal view returns (ManagedStorage storage storageRef) {
        bytes32 slot = _MANAGED_STORAGE_SLOT;

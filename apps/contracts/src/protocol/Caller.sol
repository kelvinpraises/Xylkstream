...
contract Caller is EIP712("Caller", "1"), ERC2771Context(address(this)) {
    ...
    bytes32 internal immutable CALL_SIGNED_TYPE_HASH = keccak256(bytes(CALL_SIGNED_TYPE_NAME));
    ...
    function callSigned(
        ...
        bytes memory encoded = abi.encode(
            CALL_SIGNED_TYPE_HASH, sender, target, dataHash, msg.value, currNonce, deadline
        );
        // forge-lint: disable-next-line(asm-keccak256)
        bytes32 executeHash = keccak256(encoded);

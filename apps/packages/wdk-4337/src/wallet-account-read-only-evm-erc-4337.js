...
import { Safe4337Pack, GenericFeeEstimator, PimlicoFeeEstimator } from '@safe-global/relay-kit'
...
/** @typedef {import('@safe-global/relay-kit').UserOperationReceipt} UserOperationReceipt */

/** @typedef {import('@safe-global/relay-kit').IFeeEstimator} IFeeEstimator */
 ...
 * @property {Object} [contractNetworks] - Custom contract addresses for unsupported chains. Keyed by chainId string. Each entry should have: safeSingletonAddress, safeProxyFactoryAddress, multiSendAddress, multiSendCallOnlyAddress, fallbackHandlerAddress, signMessageLibAddress, createCallAddress, simulateTxAccessorAddress.
 * @property {string} [safe4337ModuleAddress] - Override for Safe 4337 module address.
 * @property {string} [safeModulesSetupAddress] - Override for Safe modules setup address.
 */
...
export default class WalletAccountReadOnlyEvmErc4337 extends WalletAccountReadOnly {
  ...
  constructor (address, config) {
    ...
    /**
     * Cached Safe address, resolved lazily from Safe4337Pack.
     *
     * @private
     * @type {string | undefined}
     */
    this._safeAddress = undefined
  }

  /**
   * Returns the account's Safe address. Derived from the Safe4337Pack's Protocol Kit
   * instance, which correctly accounts for 4337 module setup in the CREATE2 prediction.
   *
  ...
  async getAddress () {
    if (!this._safeAddress) {
      ...
      this._safeAddress = await safe4337Pack.protocolKit.getAddress()
    }
   ...
   *
   * When `contractNetworks` is present in config, uses `@safe-global/protocol-kit`'s
   * `predictSafeAddress` which supports custom contract addresses. Otherwise uses
   * the same function without contractNetworks for standard chains.
   *
   * @param {string} owner - The safe owner's address.
   * @param {Pick<EvmErc4337WalletConfig, 'chainId' | 'safeModulesVersion' | 'provider'> & { contractNetworks?: object, safe4337ModuleAddress?: string, safeModulesSetupAddress?: string }} config - The safe configuration
   * @returns {Promise<string>} The Safe address.
   */
  static async predictSafeAddress (owner, config) {
    const { chainId, safeModulesVersion, contractNetworks, provider } = config
    const network = String(chainId)

    // Resolve 4337 module addresses (from config overrides or deployment registry)
    let safe4337ModuleAddress = config.safe4337ModuleAddress
    let safeModulesSetupAddress = config.safeModulesSetupAddress
    ...
    if (!safe4337ModuleAddress) {
      ...
      safe4337ModuleAddress = deployment?.networkAddresses[network]
    }
    ...
    if (!safeModulesSetupAddress) {
      ...
      safeModulesSetupAddress = deployment?.networkAddresses[network]
    }

    if (!safe4337ModuleAddress || !safeModulesSetupAddress) {
      throw new ConfigurationError(
        `Safe4337Module and/or SafeModuleSetup not available for chain ${network}. ` +
        'Provide safe4337ModuleAddress and safeModulesSetupAddress in config.'
      )
    }

    // Encode the enableModules call — this is part of Safe.setup() initializer
    // and affects the CREATE2 predicted address
    const enableModulesData = encodeFunctionData({
      abi: [{ name: 'enableModules', type: 'function', inputs: [{ name: 'modules', type: 'address[]' }] }],
    ...
    if (contractNetworks) {
      predictOptions.customContracts = contractNetworks[network]
    }

    return protocolKitPredictSafeAddress(predictOptions)
  }
  ...
  async _getUserOperationGasCost (txs, { amountToApprove, ...config }) {
    ...
    try {
      ...
      if (typeof safe4337Pack.getTokenExchangeRate === 'function') {
      ...
      // Official @safe-global/relay-kit does not expose getTokenExchangeRate.
      // Return gas cost in native units; callers should convert if needed.
      return gasCost
    } catch (error) {
